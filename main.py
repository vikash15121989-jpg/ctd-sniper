...
# ===== BACKTEST CONFIG =====
BACKTEST_MODE = True # False kar de to live chalega
BACKTEST_END = datetime.now().date()
BACKTEST_START = BACKTEST_END - timedelta(days=365) # 1 saal peeche

print("=== POWER SPRING HYBRID V1 BACKTEST ===", flush=True)
if BACKTEST_MODE:
    print(f"Backtest Period: {BACKTEST_START} to {BACKTEST_END}", flush=True)
else:
    print(f"Live Run: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
...
R = {
    ...
    'vol_blast_ratio': 1.5, 'adx_min': 25, 'rsi_max': 70,
...
if BACKTEST_MODE:
    ...
    for current_date in date_range:
        ...
        for pos in open_positions:
            ...
            if sl_hit and target_hit:
                exit_price = pos['SL'] # SL priority on gap down
                exit_status = 'LOSS'
            elif sl_hit:
                exit_price = pos['SL']
                exit_status = 'LOSS'
        ...
        for stock, df in stock_data.items():
            ...
            if row['Close'] < high_252 * R['52h_proximity']: continue
    ...
    for cat in ['A', 'B']:
        ...
        pf = round(abs(cat_df[cat_df['Status']=='WIN']['PnL_Rs'].sum() / cat_df[cat_df['Status']=='LOSS']['PnL_Rs'].sum()), 2) if losses else 999
        ...
        print(f"Avg Days Held: {cat_df['Days_Held'].mean():.1f}", flush=True)

    # Combined
    total = len(df_bt)
    wins = len(df_bt[df_bt['Status'] == 'WIN'])
    winrate = round(wins / total * 100, 1) if total else 0
    pf = round(abs(df_bt[df_bt['Status']=='WIN']['PnL_Rs'].sum() / df_bt[df_bt['Status']=='LOSS']['PnL_Rs'].sum()), 2) if len(df_bt[df_bt['Status']=='LOSS']) else 999
    ...
    try:
        ...
        print(f"\nBacktest data saved to GSHEET: BACKTEST_HYBRID_1Y", flush=True)
    except Exception as e:
        print(f"GSheet save failed: {e}", flush=True)

else:
    # LIVE MODE - purana code yaha chalega
    print("Live mode not implemented in this backtest version", flush=True)
